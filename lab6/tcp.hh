...
// Receivenext: Next expected sequence number from the other host
// sendNext: The next sequence number to send is this.
// SentUnAcked: The latest sequence number an acknowledgement has been received for

// sentMaxSeq: In order to limit the amouunt of data retransmitted a new state variable,
//is introduced which is assigned the highest sequence number transmitted so far


// HTTP
// POST - Submits data to be processed to a specified resourc
// Get - Gets data
...
class TCPConnection
{
 public:
  ...
  void RSTFlagReceived(); // added lab 6

  udword theOffset();// the first position in the que relative transmittQue
  byte* theFirst(); // The first byte to send in the segment relative to transmittQue
  udword theSendLength(); // The number of bytes to send in a single segment
  ...
  retransmitTimer* myTimer; // Timer for retransmit

  bool activeSocket; // added lab 6
  bool finSent;
  bool RSTFlag;


  byte* transmitQueue; // Reference to the data to be sent
  udword queueLength; // the number of data to be sent
  udword firstSeq; // the seuqnece number of the first byte in queue

  udword sentMaxSeq; // Highest sequence number transmitted so far.
  uword myWindowSize; // Offered window size for each segment
...
*% CLASS TYPE   :
*%
*% DESCRIPTION  : Used for active close.
*%
*% SUBCLASSING  : no.
*%
*%***************************************************************************/
...
*% CLASS TYPE   :
*%
*% DESCRIPTION  : Used for active close. Handles fin.
*%
*% SUBCLASSING  : no.
*%
*%***************************************************************************/
